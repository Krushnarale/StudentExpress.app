...
public class RoomDAOImpl implements RoomDAO {
    ...
    public boolean save(RoomItem room) {
        ...
        try {
            ...
            docData.put("ownerId", room.getOwnerUid() != null ? room.getOwnerUid() : "");
            ...
            docData.put("images", room.getImages() != null ? room.getImages() : new ArrayList<>());
            docData.put("available", room.isAvailable());
            docData.put("rentType", room.getRentType() != null ? room.getRentType() : "Monthly");
            docData.put("minDuration", room.getMinDuration());
            if (room.getMaxDuration() != null) docData.put("maxDuration", room.getMaxDuration());
            docData.put("securityDeposit", room.getSecurityDeposit());
            if (room.getAvailableFrom() != null) docData.put("availableFrom", room.getAvailableFrom().toString());
            if (room.getAvailableUntil() != null) docData.put("availableUntil", room.getAvailableUntil().toString());
            docData.put("availabilityStatus", room.getAvailabilityStatus() != null ? room.getAvailabilityStatus() : "AVAILABLE");
            ...
            result.get();
            System.out.println("[RoomDAOImpl] Room saved successfully to Firestore: " + room.getId() + " with ownerUid: " + room.getOwnerUid());
            return true;
        } catch (Throwable e) {
    ...
    private RoomItem parseRoomFromSnapshot(DocumentSnapshot doc) {
        ...
        if (ownerUid == null || ownerUid.trim().isEmpty()) {
            ownerUid = doc.getString("ownerId");
        }
        ...
        if (rentType != null) item.setRentType(rentType);
        Double secDep = doc.getDouble("securityDeposit");
        if (secDep != null) item.setSecurityDeposit(secDep);
        Long minDur = doc.getLong("minDuration");
        if (minDur != null) item.setMinDuration(minDur.intValue());
        Long maxDur = doc.getLong("maxDuration");
        if (maxDur != null) item.setMaxDuration(maxDur.intValue());
        String availStatus = doc.getString("availabilityStatus");
        if (availStatus != null) item.setAvailabilityStatus(availStatus);

        List<?> imagesList = (List<?>) doc.get("images");
        if (imagesList != null) {
            List<String> imgs = new ArrayList<>();
            for (Object obj : imagesList) {
                if (obj != null) imgs.add(obj.toString());
            }
            item.setImages(imgs);
        }
