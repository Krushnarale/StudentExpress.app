...
public class SessionManager {
    ...
    public void login(User user) {
        login(user, user != null ? user.getRole() : null);
    }

    public void login(User user, User.Role activeLoginRole) {
        if (user != null) {
            this.currentUser = user;
            this.uid = user.getUid();
            this.name = user.getName();
            this.email = user.getEmail();
            this.role = activeLoginRole != null ? activeLoginRole : user.getRole();
