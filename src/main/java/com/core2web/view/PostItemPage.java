...
public class PostItemPage {
    ...
    public Node getPageNode(Runnable backCallback) {
        ...
        addPhotosBox.setStyle("-fx-background-color: " + Theme.CARD_BG + "; -fx-border-color: " + Theme.BORDER_COLOR
                + "; -fx-border-radius: 10px; -fx-border-style: dashed; -fx-border-width: 2px; -fx-alignment: center;");
        ...
        catCombo.getItems().addAll("Rentals - Rooms & PG", "Rentals - Furniture", "Rentals - Electronics",
                "Buy/Sell - Books", "Buy/Sell - Electronics", "Buy/Sell - Vehicles", "Services");
        ...
        postItemBtn.setOnAction(e -> {
            ...
            if (!uploadedPath[0].isEmpty()) {
                File file = new File(uploadedPath[0]);
                if (file.exists()) {
                    com.core2web.service.CloudinaryService.UploadResult uploadRes = com.core2web.service.CloudinaryService
                            .uploadImage(file, "productImages");
                    if (uploadRes != null && uploadRes.isSuccess()) {
                        finalImagePath = uploadRes.getSecureUrl();
                    } else {
                        String base64 = com.core2web.util.ImageUtil.compressAndEncode(file);
                        if (base64 != null && !base64.isEmpty()) {
                            finalImagePath = base64;
                        }
                    }
                }
            }

            String activeUserUid = com.core2web.util.SessionManager.getInstance().getUid();
            String activeUserName = com.core2web.util.SessionManager.getInstance().getName();
            ...
            ProductItem newItem = new ProductItem(
                    ...
                    condCombo.getValue(),
                    desc,
                    activeUserName != null ? activeUserName : "Student",
                    "9876543210",
                    finalImagePath,
                    activeUserUid);
            new com.core2web.controller.ProductController().addProduct(newItem);
        ...
        mainContent.getChildren().addAll(headerBox, addPhotosBox, titleInputBox, catBox, condBox, priceBox, descBox,
                locBox, postItemBtn);
    ...
    private String formInputStyle() {
        return "-fx-background-color: " + Theme.CARD_BG + "; -fx-border-color: " + Theme.BORDER_COLOR
