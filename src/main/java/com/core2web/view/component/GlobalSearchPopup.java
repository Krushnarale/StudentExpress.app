...
public class GlobalSearchPopup extends VBox {
    ...
    public void updateSearch(String query) {
        ...
        // Search Roommates
        List<RoommateItem> roommates = repo.getRoommates();
        int rmHits = 0;
        for (RoommateItem rm : roommates) {
            if (rm.getName().toLowerCase().contains(q) || rm.getLocation().toLowerCase().contains(q) || rm.getPreference().toLowerCase().contains(q)) {
