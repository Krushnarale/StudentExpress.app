...
public class SavedItemsPage {
    ...
    public Node getPageNode(Runnable onBack) {
        ...
        refreshArr[0] = () -> {
            ...
            var repo = DataRepository.getInstance();
            var savedRooms = repo.getSavedRoomIds();
            var savedProds = repo.getSavedProductIds();
            ...
            for (ProductItem p : repo.getProducts()) {
                if (savedProds.contains(p.getId())) {
                    ...
                    removeBtn.setStyle(Theme.dangerBtnStyle());
                    removeBtn.setOnAction(ev -> {
                        repo.toggleSavedProduct(p.getId());
                        refreshArr[0].run();
                    });
